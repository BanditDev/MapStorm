export const diff = (node1: Map<string, any>, node2: Map<string, any>, isRoot = false) => {
  const tasks = new Set();
  ...
  if (isRoot) {
    node1.forEach((c, key) => {
      if (c && c.props && 'key' in c.props) {
        const f = Array.from(node2).find(([_, component]) => component.props && component.props.key === c.props.key);
